...
from jax.experimental.sparse.util import _safe_asarray, _csr_to_coo
...
def _bcsr_to_bcoo(indices: jnp.ndarray, indptr: jnp.ndarray, *,
  ...
  n_batch, n_dense, _ = _validate_bcsr_indices(indices, indptr, shape)
  ...
  if n_batch:
    row_indices = jnp.reshape(row_indices,
                              shape[:n_batch] + row_indices.shape[1:])
    col_indices = jnp.reshape(col_indices,
                              shape[:n_batch] + col_indices.shape[1:])
